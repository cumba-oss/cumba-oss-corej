...
public class RuleGenerator
{
    ...
    private net.cumba.corej.core.exec.@Nullable DatasetResolver datasetResolver;
     ...
     */
    public void setDatasetResolver(net.cumba.corej.core.exec.DatasetResolver resolver)
    {
    ...
    private GeneratedRulePackage doGenerate(IDataTable table, String domName)
    {
        ...
        net.cumba.corej.core.exec.ScopeVariableSource scopeForeign = net.cumba.corej.core.exec.ScopeVariableSource
            ...
            RuleGenerationReport report)
    {
        ...
        for (int c = 0; c < colCount; c++)
        {
            ...
            net.cumba.corej.core.model.MatchDataset match = new net.cumba.corej.core.model.MatchDataset();
            match.setName("ADSL");
            ...
            List<Rule> rules, RuleGenerationReport report)
    {
        ...
        for (Rule staticRule : scopedStaticRules)
        {
            ...
            net.cumba.corej.core.model.CheckCondition expandedCheck = net.cumba.corej.core.exec.CheckConditionTransformer
            ...
            expanded.setCheckLevels(
                    net.cumba.corej.core.model.LevelCheck.mapConditions(staticRule.getCheckLevels(),
                            c -> net.cumba.corej.core.exec.CheckConditionTransformer
            ...
            RuleGenerationReport report)
    {
        ...
        rule.setExecutability(net.cumba.corej.core.model.Executability.PARTIALLY_EXECUTABLE);
        rules.add(rule);
            ...
            DataTableMeta meta, String domain, List<Rule> rules, RuleGenerationReport report)
    {
        ...
        for (Map<String, String> dv : defVars)
        {
            ...
            for (Map<String, String> vlm : vlmEntries)
            {
                ...
                List<net.cumba.corej.core.model.CheckCondition> whereChecks = buildWhereClauseChecks(
                        conditions);
                ...
                if (vlmCodelist != null && !vlmCodelist.isEmpty())
                {
                    ...
                    if (!terms.isEmpty())
                    {
                        ...
                        if (!codedValues.isEmpty())
                        {
                            ...
                            List<net.cumba.corej.core.model.CheckCondition> allChecks = new ArrayList<>(
                                    whereChecks);
     ...
     */
    private List<net.cumba.corej.core.model.CheckCondition> buildWhereClauseChecks(
            List<Map<String, String>> conditions)
    {
        List<net.cumba.corej.core.model.CheckCondition> checks = new ArrayList<>();
    ...
    private @Nullable String describeScopeSkip(Rule r, String domName, DataTableMeta meta,
            ...
            net.cumba.corej.core.exec.@Nullable ScopeVariableSource scopeForeign)
    {

 ...
 * JSON format under this module's {@code dictionaries/}<i>type</i>{@code .json}. A dictionary
 * reduces every dictionary conformance check to a lookup:
 *
 * <ul>
 ...
 *
 * <p>
 * Small dummy maps are checked into the repository under this module's {@code dictionaries/} so the
 * dictionary conformance rules execute and the {@code .cdt} scenarios discriminate without any
 * licensed dictionary data present; real dictionary data is installed by the operator behind the
 * same file format ({@code plans/done/PLAN-dictionary-seeder.md}).
 * </p>
 *
 * <p>
 ...
 * independently to audit the preferred-case contract. Nothing outside {@code corej-core} reads it.
 * (Before {@code 987ab2ba1} this javadoc claimed the files were also read by a Python reference
 * engine's {@code ValueMapValidator} shim; that module was renamed away and no such reader exists —
 * the claim wrongly implied a cross-engine constraint on the format.)
 * </p>
 *
 * <p>

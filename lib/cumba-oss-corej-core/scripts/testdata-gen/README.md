...
rule engine against domains the existing test studies don't cover. Designed in
`plans/done/PLAN-sdtm-testdata-gen.md`, which is kept in the internal monorepo and
...
**Status:** the design plan `plans/done/PLAN-sdtm-testdata-gen.md` (internal monorepo
only) is **complete (Phases 0–7)** and independently verified. Both lanes (SDTMIG-3.4,
...
**Gotcha — the rule corpus drifts.** `lib/corej-rules/target/rules/*.json` are
regenerated on every build (they changed several times mid-build).
...
- **Java↔Python parity (Phase 6 stretch) — HISTORICAL, never performed and no
  ...
  (`lib/corej-rules/rulespec/`) — there is no second engine to compare
  against.

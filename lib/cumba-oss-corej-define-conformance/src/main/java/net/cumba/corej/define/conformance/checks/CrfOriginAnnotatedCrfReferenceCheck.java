 ...
 * absent/blank is ignored (the XSD requires leafID; schema defects are the pre-pass's beat), and an
 * Origin whose DocumentRefs are ALL ignored that way is satisfied rather than fired — it is
 * indistinguishable from a document with no leafIDs to compare. With no AnnotatedCRF in reach the
 * set is empty and every qualifying Origin carrying a usable leafID fires — nothing to match, per
 * the row.
 * </p>
 */
